...
# latest script version: 2012-03-23
...
# definition of download cell
mcell <- "h24v05"

# definition of area-specific MODIS conversion factors
MODIS_UL <- 'SPATIAL_SUBSET_UL_CORNER = ( 320000.0 3900000.0 )'
MODIS_LR <- 'SPATIAL_SUBSET_LR_CORNER = ( 570000.0 3660000.0 )'
MODIS_UTMZ <- 'UTM_ZONE = 43'
...
# basic settings of remote site (this should not change, except for anonymous user name)
MOD10A1 <- "ftp://n4ftl01u.ecs.nasa.gov/MOST/MOD10A1.005/" # ftp data source for MODIS snow files
MOD10A1a <- "ftp://anonymous:test@n4ftl01u.ecs.nasa.gov/MOST/MOD10A1.005/" # log-in to ftp server

# get the list of directories (code thanks to Barry Rowlingson):
items <- strsplit(getURL(MOD10A1), "\n")[[1]] # get listing from ftp server - this may take some time!
...
d2 <- min(d2, nrow(dates)) # adjust to available data
print(paste("Download starts with",as.character(dates[d1,1])))

# get the list of files:
dates$BLOCK1 <- rep(NA, length(dates$dirname))
dlpattern <- paste("MOD10A1",mcell,"hdf",sep=".*.")

# start loop over directories (each directory contains files for a single day)
for(a in d1:d2){ 

  getlist <- strsplit(getURL(paste(MOD10A1, dates$dirname[[a]], "/", sep=""), .opts=curlOptions(ftplistonly=TRUE)), "\r\n")[[1]]
  
  # set filter for hdf file (but includes xml also!)
  BLOCK1 <- getlist[grep(getlist, pattern=dlpattern)[1]]
  if (is.na(BLOCK1)) next # if file does not exist, don't try to download
  # remove "." from the file name:
  dirname1 <- sub(sub(pattern="\\.", replacement="_", dates$dirname[[a]]), pattern="\\.", replacement="_", dates$dirname[[a]])
  
  # this downloads the individual file
  download.file(paste(MOD10A1a, dates$dirname[[a]], "/", BLOCK1,sep=""), destfile=paste(HDFd, dirname1, ".hdf", sep=""), mode='wb', method='wget', quiet=T, cacheOK=FALSE)
  ...
  write('  ', filename, append=TRUE)
  write(MODIS_UL, filename, append=TRUE) 
  write(MODIS_LR, filename, append=TRUE)
  write('  ', filename, append=TRUE) 
  ...
  write('  ', filename, append=TRUE) 
  write(MODIS_UTMZ, filename, append=TRUE) 
  write('  ', filename, append=TRUE)
