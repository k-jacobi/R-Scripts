...
# Use of Tcrit with values obtained through calibration
# script version: 13/09/2011
...
tfile <- "swe_db_2009.txt"
a_tab <- read.table("a_list.txt", header=TRUE, as.is=T) # 10-day a-values for Mangla
...
for (y in year1:year2){
    ...
    for (d in dd1:dd2){
        ...
        a <- subset(a_tab[,3], a_tab[,1]==M & a_tab[,2]==D, select=a_tab[,3])
        ...
        Ras <- max(Ras+Rds-a*10*Rdd, R0) # SWE accumulation - melt balance - factor 10 because of cm
    ...
    writeRaster(Ras,paste(y,"amSWE_09.tif",sep="_"),overwrite=T, datatype="INT2U")
