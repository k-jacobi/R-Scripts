...
if (varid=="tmp2m"){
    ...
    vdiv <- 2 # average
    vlim <-  c(-20,30)
    vnames <- c('min','max','avg','movavg')
} else {
    ylab <- "Precipitation in mm"
    ylabi <- "Precipitation"
    vfact <- 43200 # to convert from p-rate to p-total
    vdiv <- 1 # total
    vlim <-  c(0,50)
    vnames <- c('t1','t2','total','movavg')
}
...
for (d in date1:date2){
    ...
    for (n in 1:4){
        ...
        t <- extract(tmp_brick, cellFromXY(tmp_brick, c(74,34)))*vfact # time series
        
        # get start time/date
        t0 <- as.POSIXct(substr(gribfile,10,19), format="%Y%m%d%H", tz = "GMT")
        days <- nlayers(tmp_brick)-nlayers(tmp_brick)%%2
        ti <- seq(t0, by= t0-(t0-43200), len=days) # shift 12 hours
        ...
        if (varid=="tmp2m"){
            plot (d.df[,1:2], type="l", col="red", xlim=xlim, ylim=vlim, xlab="", ylab=ylab, main=tmain)            
            par(new=T)
            plot (x=d.df[,1], y=d.df[,3], col="blue", type="l", xlim=xlim, ylim=vlim, xlab="", ylab="") 
            par(new=T)
            plot (x=d.df[,1], y=d.df[,4], col="green", type="l", lwd=1, xlim=xlim, ylim=vlim, xlab="", ylab="")
            par(new=T)
            plot (x=d.df[,1], y=d.df[,5], col="darkgreen", type="l", lwd=2, xlim=xlim, ylim=vlim, xlab="", ylab="")
        } else {
            plot (x=d.df[,1], y=d.df[,4], col="blue",type="l",lwd=1,xlim=xlim,ylim=vlim,xlab="",ylab=ylab,main=tmain)
            par(new=T)
            plot (x=d.df[,1], y=d.df[,5], col="darkgreen", type="l", lwd=2, xlim=xlim, ylim=vlim, xlab="", ylab="")
        }
        grid(nx=NA, ny=NULL)
        ...
        if (varid=="tmp2m"){
            legend(title="daily val.",x=xlim[2]+14,y=vlim[2]+2,legend=c("max","avg","mova","min"),lty=1,lwd=c(1,1,2,1), col=c("red","green","darkgreen","blue"))
        } else {
            legend(title="daily val.",x=xlim[2]+14,y=vlim[2]+2,legend=c("total","mova"),lty=1,lwd=c(1,2),col=c("blue","darkgreen"))
        }
